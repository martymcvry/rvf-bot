...
alias _rad.deactiveer {
  if (%rad.active == yes) {
    ...
    if (%voice.rad == on) { .mode %main.channel - [ $+ [ %rad.voicecmd ] ] [ %rad.voice ] }
    msg %main.channel  [ $+ [ %kleuren-rad.copyright ] $+ ] (c) 2005 - Tinke_lat (tinke_lat@skynet.be)
...
alias -l _rad_kand.disable {
  ...
  if (%rad.numkand == 0) {
    ...
    set %rad.kand.counter 1
    set %rad.kand.count 2
    ...
    while (%rad.kand.count <= %rad.numkand) {
      msg %main.channel  [ $+ [ %kleuren-rad.kandidatenlijst ] $+ ] Kandidaat [ %rad.kand.count ] $+ :  $+ [ %rad.kandidaat. [ $+ [ %rad.kand.count ] ] ] $+ 
      inc %rad.kand.count
    }
      ...
      if (%voice.rad == on) {
        ...
        while (%voice.counter <= %rad.numkand) {
          if (%rad.kandidaat. [ $+ [ %voice.counter ] ] !isvoice %main.channel) {
            set %rad.voice [ %rad.voice ] [ %rad.kandidaat. [ $+ [ %voice.counter ] ] ]
            set %rad.voicecmd [ [ %rad.voicecmd ] $+ ] v
            inc %voice.count
          }
          inc %voice.counter
        }
        .mode %main.channel + [ $+ [ %rad.voicecmd ] ] [ %rad.voice ]
      }
